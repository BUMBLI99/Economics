...
%
%% - Variables en nivel, tasas, brechas o inflaci贸n: usar aditivos.
%   Ejemplo:
%      VIX_alt = VIX_base + vix_add
%
%% Punto clave:
%% - Multiplicador = 1  => no se impone la variable en ese per铆odo.
% - Aditivo = 0        => no se impone la variable en ese per铆odo.
%% - Si pones un shock solo en 2026Q3, solo se exogeniza 2026Q3.
% - Los per铆odos siguientes quedan end贸genos y los resuelve el modelo.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist('config_ipom', 'file') == 2
    ...
    baselineFile = cfg_ipom.baselineFile;
    outputFile   = fullfile(cfg_ipom.rawOutputDir, 'fcast_alt_tpm45_2026.csv');
    outputFileGeneric = cfg_ipom.altScenarioGenericFile;
else
    baselineFile = 'fcast_ipom_exact.csv';
    outputFile   = 'fcast_alt_tpm45_2026.csv';
...
altname        = 'Escenario TPM 4.5 durante 2026';
country        = 'Chile';
exchange       = 'CHL/USA';

if exist('cfg_ipom','var') && isfield(cfg_ipom, 'reportDir')
    reportName = fullfile(cfg_ipom.reportDir, 'Escenario_TPM45_2026');
else
    reportName = 'Escenario_TPM45_2026';
end
...
if exist(baselineFile,'file') ~= 2
    error('No existe %s. Primero corre step01_identificar_shocks_ipom.m', baselineFile);
end
...
%% ============================================================
%% 2. BLOQUE DE MANIPULACI脫N DEL ESCENARIO
% ============================================================
...
% Si una variable queda en 1 o 0 en todos los per铆odos, NO se exogeniza.
...
%% ------------------------------------------------------------
%% 2.2 Aditivos: niveles, tasas, brechas e inflaci贸n
%% ------------------------------------------------------------
...
%{
%% === OPCION 1: objetivo directo para el nivel del TCR ===

lz_dates = qq(2026,1):qq(2027,4);
...
% Baseline en nivel: si L_Z = 100*log(TCR), entonces TCR = exp(L_Z/100)
lz_base_index = exp(h.L_Z(lz_dates)/100);
lz_base_index = reshape(lz_base_index, 1, []);
% Multiplicador relativo al baseline
lz_mult_values = lz_index_target ./ lz_base_index;
lz_mult_path = local_set_path( ...
    lz_mult_path, altRange, lz_dates, lz_mult_values ...
);



%% === OPCION 1: objetivo directo para CRECSC ===

crec_dates = qq(2026,2):qq(2027,4);
...
crec_add_values = local_level_to_additive( ...
    h, 'CRECSC', crec_dates, crec_target ...
);
crec_add_path = local_set_path( ...
    crec_add_path, altRange, crec_dates, crec_add_values ...
);
%}
...
%% ------------------------------------------------------------
%% E. IPC observado 2026Q1 y 2026Q2 - consistente con Excel
%% ------------------------------------------------------------
% Convencion correcta:
% - Los datos 2.6611 y 3.3998 son variaciones porcentuales
%   convencionales de indices promedio trimestrales.
% - El modelo usa L_CPI = 100*log(indice).
% - Por eso primero se trabaja en niveles de indice y despues se pasa a log.
%
% Importante:
% - Q1 se impone usando el crecimiento anual convencional del promedio
%   trimestral.
% - Q2 se construye como promedio de los niveles mensuales abril-mayo-junio,
%   no como punta a punta junio/marzo.

q1_date = qq(2026,1);
q2_date = qq(2026,2);

%% === 2026Q1: IPC total y core observados ===

d4lcpi_2026q1_actual_pct    = 2.6611;
d4lcpixfe_2026q1_actual_pct = 3.402;

% Baseline/observado del mismo trimestre del a駉 anterior.
% Si h.L_CPI es el log del promedio trimestral, esto es correcto.
cpi_q1_2025_avg_index    = exp(h.L_CPI(q1_date-4)/100);
cpixfe_q1_2025_avg_index = exp(h.L_CPIXFE(q1_date-4)/100);

% Nivel promedio trimestral 2026Q1 compatible con el dato anual observado.
cpi_q1_2026_avg_index = cpi_q1_2025_avg_index * ...
    (1 + d4lcpi_2026q1_actual_pct/100);

cpixfe_q1_2026_avg_index = cpixfe_q1_2025_avg_index * ...
    (1 + d4lcpixfe_2026q1_actual_pct/100);

% Pasar a 100*log(indice), que es la unidad del modelo.
lcpi_q1_target    = 100*log(cpi_q1_2026_avg_index);
lcpixfe_q1_target = 100*log(cpixfe_q1_2026_avg_index);

% DLA compatible con esos niveles trimestrales.
dla_cpi_q1_target = 4 * (lcpi_q1_target - h.L_CPI(q1_date-1));

dla_cpixfe_q1_target = 4 * ...
    (lcpixfe_q1_target - h.L_CPIXFE(q1_date-1));


%% === 2026Q2: IPC total mensual observado ===

% Nivel mensual observado de marzo 2026.
% En tu Excel aparece aproximadamente:
%   IPC marzo 2026 = 110.753006
%
% Si cambias la base/fuente, actualiza este n鷐ero.
cpi_mar_2026_index = 110.753006;

% Variaciones mensuales convencionales, no logar韙micas.
ipc_q2_monthly_pct = [ ...
     1.3, ... % abril
     0.3, ... % mayo
    -0.1  ... % junio
...
% Construir niveles mensuales.
cpi_apr_2026_index = cpi_mar_2026_index * (1 + ipc_q2_monthly_pct(1));
cpi_may_2026_index = cpi_apr_2026_index * (1 + ipc_q2_monthly_pct(2));
cpi_jun_2026_index = cpi_may_2026_index * (1 + ipc_q2_monthly_pct(3));

% IPC trimestral = promedio de niveles mensuales.
cpi_q2_2026_avg_index = mean([ ...
    cpi_apr_2026_index, ...
    cpi_may_2026_index, ...
    cpi_jun_2026_index ...
]);

% Pasar a 100*log(indice).
lcpi_q2_target = 100*log(cpi_q2_2026_avg_index);

% DLA compatible con el promedio trimestral.
dla_cpi_q2_target = 4 * (lcpi_q2_target - lcpi_q1_target);

% Inflaci髇 anual convencional impl韈ita de Q2, solo para chequeo.
cpi_q2_2025_avg_index = exp(h.L_CPI(q2_date-4)/100);

d4lcpi_q2_impl_pct = 100 * ...
    (cpi_q2_2026_avg_index / cpi_q2_2025_avg_index - 1);

d4lcpi_q2_impl_log = 100 * ...
    log(cpi_q2_2026_avg_index / cpi_q2_2025_avg_index);


%% === Imponer en el modelo v韆 DLA_CPI y DLA_CPIXFE ===

ipc_dates = [q1_date, q2_date];
...
dla_cpi_add_values = local_level_to_additive( ...
    h, 'DLA_CPI', ipc_dates, dla_cpi_targets ...
);

dla_cpi_add_path = local_set_path( ...
    dla_cpi_add_path, altRange, ipc_dates, dla_cpi_add_values ...
);


% IPC subyacente / sin volatiles Q2 2026
% Supuesto central: Abr 0.5%, May 0.2%, Jun 0.2%
% Esto equivale aproximadamente a una inflacion trimestral anualizada
% del IPC sin volatiles de 4.55% en 2026Q2.

dla_cpixfe_q2_target = 4.25;

core_dates = [q1_date, q2_date];

dla_cpixfe_targets = [ ...
    dla_cpixfe_q1_target, ...
    dla_cpixfe_q2_target ...
];

dla_cpixfe_add_values = local_level_to_additive( ...
    h, 'DLA_CPIXFE', core_dates, dla_cpixfe_targets ...
);

dla_cpixfe_add_path = local_set_path( ...
    dla_cpixfe_add_path, altRange, core_dates, dla_cpixfe_add_values ...
);

%% ============================================================
%% F. TPM = 4.5% durante 2026 y retorno al IPOM desde 2027
%% ------------------------------------------------------------
% Solicitud: mantener todo el resto igual y fijar la TPM en 4.5%
% durante todo 2026.
%
% Interpretacion operativa adicional:
% desde 2027Q1 en adelante, la TPM debe volver directamente a la
% trayectoria del escenario IPOM/baseline. Por eso no basta con dejarla
% libre desde 2027: si se deja libre, la regla de politica puede reaccionar
% a efectos rezagados del shock de 2026 y la TPM podria no coincidir con
% el IPOM. Aqui se impone explicitamente:
%   - 2026Q1:2026Q4 = 4.5
%   - 2027Q1:endfcast = TPM del baseline h
%
% Tecnicamente: TPM es una variable en nivel/tasa, por eso se impone
% con un aditivo igual a target - baseline. El shock endogeno usado es
% SHK_TPM, ya incorporado mas abajo en el catalogo.
%
% Si quieres respetar 2026Q1 como dato historico/observado y solo imponer
% desde 2026Q2, cambia la linea de tpm_dates_2026 a:
%   tpm_dates_2026 = qq(2026,2):qq(2026,4);

% Forzar todos los vectores como fila. IRIS puede devolver h.TPM(rango)
% como columna dependiendo de la version/forma interna del tseries.
tpm_dates_2026  = reshape(qq(2026,1):qq(2026,4), 1, []);
tpm_target_2026 = 4.5 * ones(1, numel(tpm_dates_2026));

% Retorno exacto al baseline/IPOM desde 2027Q1 hasta el final del horizonte.
tpm_dates_return  = reshape(qq(2027,1):endfcast, 1, []);
tpm_target_return = reshape(h.TPM(tpm_dates_return), 1, []);

tpm_dates  = [tpm_dates_2026, tpm_dates_return];
tpm_target = [tpm_target_2026, tpm_target_return];

if numel(tpm_dates) ~= numel(tpm_target)
    error('TPM path invalido: fechas y valores tienen distinto largo.');
end
...
%% ============================================================
%% 3. Cat谩logo de variables del escenario
%% ============================================================
%
% Formato:
...
% tipo = 'add'  para variables en niveles/tasas/brechas/inflaci贸n
...
% TPM se impone manualmente con local_force_level_path, no mediante paths neutrales.
% Motivo: desde 2027 el target es igual al baseline, por lo que el aditivo es cero;
% si pasara por local_apply_scenario, esos periodos no se exogenizarian.
...
% TPM: forzar la trayectoria completa, incluso cuando el target coincide
% exactamente con el baseline/IPOM. Esto es indispensable para que desde
% 2027 la TPM vuelva al IPOM y no quede endogena por la regla de politica.
[d, planItems] = local_force_level_path( ...
    h, d, planItems, 'TPM', 'SHK_TPM', tpm_dates, tpm_target ...
);
...
function path = local_set_path(path, fullRange, dates, values)
    ...
    for k = 1:numel(dates)
        ...
        if isempty(idx)
            error('local_set_path: la fecha %s no est谩 dentro de altRange.', local_dat2char(dates(k)));
        end
...
function out = local_take_path(path, n)
    ...
    if isempty(path)
        error('local_take_path: path vac铆o.');
    end
...
function [d, planItems] = local_force_level_path(h, d, planItems, varName, shockName, dates, targetLevel)
% Fuerza una variable en nivel/tasa exactamente al target indicado.
%
% A diferencia de local_apply_scenario(), esta funcion NO descarta los
% periodos en que target - baseline = 0. Esto permite exogenizar una
% variable para que coincida exactamente con el baseline en cierto rango.
% Caso relevante: TPM = 4.5 durante 2026 y TPM = baseline/IPOM desde 2027.

    dates = reshape(dates, 1, []);
    targetLevel = reshape(targetLevel, 1, []);

    if isscalar(targetLevel) && numel(dates) > 1
        targetLevel = repmat(targetLevel, 1, numel(dates));
    end

    if numel(targetLevel) ~= numel(dates)
        error('local_force_level_path: targetLevel no coincide con dates.');
    end

    if ~isfield(h, varName)
        error('local_force_level_path: no existe %s en el baseline.', varName);
    end

    if ~(isfield(h, shockName) || isfield(d, shockName))
        error('local_force_level_path: no existe el shock %s para imponer %s.', shockName, varName);
    end

    baseShape = h.(varName)(dates);
    d.(varName)(dates) = reshape(targetLevel, size(baseShape));

    planItems(end+1).var   = varName; %#ok<AGROW>
    planItems(end).shock   = shockName;
    planItems(end).dates   = dates;
    planItems(end).type    = 'force-level';

    fprintf('[FORZADO] %-14s con %-18s | %s a %s | n = %d\n', ...
        varName, shockName, local_dat2char(dates(1)), ...
        local_dat2char(dates(end)), length(dates));
end
...
function [d, planItems] = local_apply_scenario(h, d, catalog, altRange)
    ...
    for i = 1:size(catalog,1)
        ...
        if isempty(activeIdx)
            fprintf('[NEUTRO]  %-14s queda end贸gena.\n', v);
            continue
        end
...
function simplan = local_build_plan(m, fcastrange, planItems)
    ...
    fprintf('\n============================================================\n');
    fprintf('Plan de simulaci贸n\n');
    fprintf('============================================================\n');

    if isempty(planItems)
        warning('No hay variables exogenizadas. El escenario ser谩 igual al baseline salvo cambios externos ya cargados.');
        return
    end
