...
%
%% - Variables en nivel, tasas, brechas o inflaciÃ³n: usar aditivos.
%   Ejemplo:
%      VIX_alt = VIX_base + vix_add
%
%% Punto clave:
%% - Multiplicador = 1  => no se impone la variable en ese perÃ­odo.
% - Aditivo = 0        => no se impone la variable en ese perÃ­odo.
%% - Si pones un shock solo en 2026Q3, solo se exogeniza 2026Q3.
% - Los perÃ­odos siguientes quedan endÃ³genos y los resuelve el modelo.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist('config_ipom', 'file') == 2
    ...
    outputFile   = cfg_ipom.altScenarioFile;
...
if exist('cfg_ipom','var') && isfield(cfg_ipom, 'reportDir')
    reportName = fullfile(cfg_ipom.reportDir, 'Escenario_Petroleo_Gap');
else
...
%% ============================================================
%% 2. BLOQUE DE MANIPULACIÃ“N DEL ESCENARIO
% ============================================================
...
% Si una variable queda en 1 o 0 en todos los perÃ­odos, NO se exogeniza.
...
%% ------------------------------------------------------------
%% 2.2 Aditivos: niveles, tasas, brechas e inflaciÃ³n
%% ------------------------------------------------------------
...
d4lcpixfe_2026q1_actual_pct = 3.3998;

% Baseline/observado del mismo trimestre del año anterior.
...
%
% Si cambias la base/fuente, actualiza este número.
cpi_mar_2026_index = 110.753006;

% Variaciones mensuales convencionales, no logarítmicas.
ipc_q2_monthly_pct = [ ...
     1.3, ... % abril
     0.3, ... % mayo
    -0.2  ... % junio
...
% Inflación anual convencional implícita de Q2, solo para chequeo.
...
%% === Imponer en el modelo vía DLA_CPI y DLA_CPIXFE ===
...
dla_cpixfe_targets = dla_cpixfe_q1_target;
...
%% ============================================================
%% 3. CatÃ¡logo de variables del escenario
%% ============================================================
%
% Formato:
...
% tipo = 'add'  para variables en niveles/tasas/brechas/inflaciÃ³n
...
function path = local_set_path(path, fullRange, dates, values)
    ...
    for k = 1:numel(dates)
        ...
        if isempty(idx)
            error('local_set_path: la fecha %s no estÃ¡ dentro de altRange.', local_dat2char(dates(k)));
        end
...
function out = local_take_path(path, n)
    ...
    if isempty(path)
        error('local_take_path: path vacÃ­o.');
    end
...
function [d, planItems] = local_apply_scenario(h, d, catalog, altRange)
    ...
    for i = 1:size(catalog,1)
        ...
        if isempty(activeIdx)
            fprintf('[NEUTRO]  %-14s queda endÃ³gena.\n', v);
            continue
        end
...
function simplan = local_build_plan(m, fcastrange, planItems)
    ...
    fprintf('\n============================================================\n');
    fprintf('Plan de simulaciÃ³n\n');
    fprintf('============================================================\n');

    if isempty(planItems)
        warning('No hay variables exogenizadas. El escenario serÃ¡ igual al baseline salvo cambios externos ya cargados.');
        return
    end
