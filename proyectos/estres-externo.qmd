...
Para reducir ese problema, el pipeline ahora puede construir un chequeo pseudo out-of-sample con ventana expansiva: cada predicción se estima usando solo datos anteriores a la fecha evaluada. Esta versión es más exigente y menos suave, pero se acerca mejor a la pregunta operacional: *qué habría señalado el monitor con la información disponible en ese momento*.
...
if (nrow(oos_stress) > 0) {
  ...
  cat("Ejecuta `source(\"scripts/06_update_estres_financiero.R\")` para generar `stress_index_chile_oos.csv` y activar esta tabla.")
}
```
